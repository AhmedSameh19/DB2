...
public class DBApp {
	...
	public void init() throws DBAppException {
		...
		try {
			...
			props.load(input);
			int size = Integer.parseInt(props.getProperty("MaximumRowsCountinPage"));
			if (size < 4) {
				throw new DBAppException("MaximumRowsCountinPage must be more than 3 it's value: " + size);
			} else {
	...
	public void createTable(String strTableName,
		...
		if (tableNames.contains(strTableName)) {
			...
			for (String key : htblColNameType.keySet()) {
				...
				if (!(value.equals("java.lang.double") || value.equals("java.lang.String")
			...
			try {
				...
				for (String myKey : htblColNameType.keySet()) {
					...
					metaData.add(
							new String[] { strTableName, myKey, myValue, String.valueOf(clusterKey), "null", "null" });
	...
	public void createIndex(String strTableName, String strColName, String strIndexName) throws DBAppException {
		try {
			overRideCSV(strTableName, strColName, strIndexName);
			Table t = getTable(strTableName);
			t.indexing(strColName, strIndexName);
			saveTable(t);
		} catch (Exception e) {
			throw new DBAppException("Error while Indexing: " + e.getMessage());
		}
	}

	public void overRideCSV(String strTableName, String strColName, String strIndexName) throws DBAppException {
		BufferedReader br = null;
		BufferedWriter bw = null;
		String itemsPath = "metadata.csv";
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				String columnName = data[1].trim();
				if (tableName.equalsIgnoreCase(strTableName) && columnName.equalsIgnoreCase(strColName)) {
					data[4] = strIndexName;
					data[5] = "B+Tree";
			...
			if (!flag)
				throw new DBAppException("This column doesn't exist in this table!");
			bw = new BufferedWriter(new FileWriter(itemsPath));
	...
	public void insertIntoTable(String strTableName,
		...
		try {
			...
			if (!(t1.inTable(t))) {
				...
				throw new DBAppException("Error inserting record change the PK already exists ");
			}
	...
	public static void verifyRow(String strTableName, Hashtable<String, Object> htblColNameValue)
	...
	public static void main(String[] args) throws DBAppException, IOException {
		...
		try {
			...
			htblColNameValue.put("name", new String("John sakr"));
		...
		try {
			...
			System.out.println(dbApp);
			htblColNameValue.clear();
			htblColNameValue.put("name", new String("abdo"));
			htblColNameValue.put("gpa", Double.valueOf(2.33));

			dbApp.updateTable(strTableName,"453455", htblColNameValue);
			System.out.println(dbApp);
		} catch (Exception e) {
		...
		// dbApp.insertIntoTable( strTableName , htblColNameValue );
		// htblColNameValue.clear( );
		// htblColNameValue.put("id", Integer.valueOf( 83942 ));
		// htblColNameValue.put("name", new String("bash" ) );
		// htblColNameValue.put("gpa", Double.valueOf( 2.1 ) );
		// dbApp.insertIntoTable( strTableName , htblColNameValue );
		// dbApp.createIndex(strTableName, "id", "idIndex" );
		// System.out.println(dbApp);
		// System.out.println(readCsv(strTableName));
	}
